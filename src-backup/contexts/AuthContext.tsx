...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const appId = 76014;
  const redirectUrl = 'https://primetrades.app';

  useEffect(() => {
    // Initialize API connection
    const connection = new DerivAPI({ app_id: appId });
    setApi(connection);
  ...
  const login = () => {
    const oauthUrl = `https://oauth.deriv.com/oauth2/authorize?app_id=${appId}&l=en&redirect_uri=${encodeURIComponent(redirectUrl)}`;
